...
func HanlderUpdateRecord(db *gorm.DB) func(ctx *gin.Context) {
	return func(ctx *gin.Context) {
		...
		bizz := biz.NewUpdateRecord(store)

		data := model.UpdateStructToDB{
			Updated: time.Now().UnixMilli(),
			Session: newRequest.Session.String(),
			UnVote:  true,
		}

		if err := bizz.UpdateNewRecord(ctx, data); err != nil {
			log.Printf(`SoulT7 - [%s] - Create record fail with err: %s`, id, err)
			ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error(), "message": "Decrease vote fail", "status": false, "data": nil})
			return
		}

		ctx.JSON(http.StatusOK, gin.H{"error": "", "message": "Decrease vote success", "status": true, "data": nil})
		return
	}
}
